...
export default function Recommendation() {
  const data = useSelector((state: any) => state.Recommendations.data.results);
      ...
      {data.slice(0, 10).map((item: any, index: any) => (
        ...
        >
          <Card
            className={
              isHovered == item.id
                ? "scale-105 duration-500 ease-in"
                : "scale-100 duration-500 ease-out"
            }
          >
            <CardContent
              className={`w-full h-full aspect-video items-center justify-center p-0 rounded-sm `}
            >
              <div className="relative">
                ...
                <div
                  className={`absolute bottom-0 w-full h-2/4 bg-black/50 flex gap-3 justify-between p-2 ${
                    isHovered == item.id
                      ? "visible duration-500 ease-in"
